...
def formatting_sequence(sequence, ec_label, control_tag=None):
    """
    Formats sequence with optional control tag for preference learning
    """
    if control_tag:
        # Format: control_tag<sep>ec_label<sep><start>sequence<end>
        return f"{control_tag}<sep>{ec_label}<sep><start>{sequence}<end><|endoftext|>"
    else:
        # Original format
...
def generate_dataset(iteration_num, ec_label, mode, control_tag=None):
    ...
    # Still read sequences generated by original seq_gen.py
    ...
    for entry in sequences_rep:
        ...
        # Compute stability-based reward instead of length
        stability_score = compute_stability(sequence)
        
        # Create input with control tag based on stability
        if stability_score > high_threshold:
            formatted_seq = formatting_sequence(sequence, ec_label, "<stability=high>")
            weight = 1.0  # Positive preference
        else:
            formatted_seq = formatting_sequence(sequence, ec_label, "<stability=low>")
            weight = 0.0  # Negative preference
            
        data["sequence"].append(formatted_seq)
