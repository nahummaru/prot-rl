...
def batched_perplexity_from_logits(logits: torch.Tensor,
                                   labels: torch.Tensor,
                                   attention_mask: torch.Tensor) -> torch.Tensor:
    """
    logits          : (B, L, V) – raw decoder outputs
    ...
    Returns a tensor of perplexities for each batch.
    """
    ...
    # Step 3: mask padding, average, exponentiate
    nll = nll * shift_mask                               # zero-out pads
    sum_nll = nll.sum(dim=1)                             # sum over sequence length
    sum_mask = shift_mask.sum(dim=1)                     # sum over sequence length
    mean_nll = sum_nll / sum_mask                        # average over real tokens
    ppl = torch.exp(mean_nll)                            # exponentiate to get perplexity
