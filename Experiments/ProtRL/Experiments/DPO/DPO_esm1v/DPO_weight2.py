...
def generate_dataset(iteration_num, ec_label):
     ...
     sequences_rep = compute_esm_log_likelihood(sequences_rep,ec_label)
     ...
     for entry in alpha_TM_score:
            ...
            esv1 = sequences_rep[str(name)]['esmv1']
            lenght_rew = math.exp(-((((220/len(sequence))-1)**2)/(0.5**2))) # Gaussian center on 1. The closer the ratio between len and aligment is, the higher is the reward
...
def compute_esm_log_likelihood(sequences_rep, ec_label):
    with open(f'metrics_{ec_label}_iteration{iteration_num-1}.txt','r') as f:
        data = f.readlines()
        for entry in data[1:]:
            name, ems1v = entry.split(',')
            sequences_rep[str(name)]['esmv1'] = float (ems1v)
