...
def main(label, model, special_tokens, device, tokenizer):
    ...
    outputs = model.generate(
        ...
        pad_token_id=0,
        do_sample=True,
        num_return_sequences=5)  # Reduced from 20 to 5
...
if __name__=='__main__':
    ...
    parser.add_argument("--n_batches", type=int, default=1)  # For testing, default to 1 batch
    ...
    n_batches = args.n_batches # For testing, use command line argument
    print(f"Generating {n_batches} batch(es) of 20 sequences each")
        ...
        for key, value in sequences.items():
            for index, val in enumerate(value):
                if all(char in canonical_amino_acids for char in val[0]): # remove sequences with characters out of the 20 canonical amino acids 
                    print(f"Valid sequence {index+1}: {val[0][:20]}... (perplexity: {val[1]:.2f})")
